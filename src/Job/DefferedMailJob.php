<?php
namespace Josegonzalez\CakeQueuesadilla\Job;
...
class DeferredEmailJob
{
    ...
    public function send()
    {
        ...
        $this->email->setProfile($this->profile());
        $this->email->setTransport($this->transport());
        ...
        if ($this->test) {
            $this->email->setTo($this->testEmailAddress());
        }

        try {
            if (!empty($this->viewVars)) {
                $this->email->setViewVars($this->viewVars);
            }
